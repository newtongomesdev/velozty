import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { useAuth } from "../components/auth/AuthGuard";
import { useI18n } from "../components/i18n/I18nProvider";
import { useToast } from "../components/ui/Toast";
import { Button } from "../components/ui/Button";
import { Card, CardTitle } from "../components/ui/Card";
import { ShieldCheck, UserPlus, Flame, Cpu } from "lucide-react";

const LGPD_CONSENT_KEY = "velozty_lgpd_consent";
const TERMS_CONSENT_KEY = "velozty_terms_privacy_consent";

export const Login: React.FC = () => {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [displayName, setDisplayName] = useState("");
  const [isSignUp, setIsSignUp] = useState(false);
  const [authLoading, setAuthLoading] = useState(false);
  const [acceptedTerms, setAcceptedTerms] = useState(false);
  
  const { user, signInUser, signUpUser, loginWithProvider, requestPasswordReset } = useAuth();
  const { showToast } = useToast();
  const { t } = useI18n();
  const navigate = useNavigate();

  // Redirect if user is already logged in
  useEffect(() => {
    if (user) {
      navigate("/dashboard");
    }
  }, [user, navigate]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim()) {
      showToast(t("login.invalidEmail"), "warning");
      return;
    }

    if (!password.trim()) {
      showToast(t("login.passwordRequired"), "warning");
      return;
    }

    if (isSignUp && password !== confirmPassword) {
      showToast(t("login.passwordMismatch"), "warning");
      return;
    }

    if (isSignUp && !acceptedTerms) {
      showToast(t("login.termsRequired"), "warning");
      return;
    }

    setAuthLoading(true);
    try {
      if (isSignUp) {
        const result = await signUpUser(email, password, displayName);
        const acceptedAt = new Date().toISOString();
        localStorage.setItem(LGPD_CONSENT_KEY, "accepted");
        localStorage.setItem(TERMS_CONSENT_KEY, JSON.stringify({
          accepted: true,
          accepted_at: acceptedAt,
          email,
          version: "2026-05-26",
        }));
        showToast(result.needsEmailConfirmation ? t("login.confirmationRequired") : t("login.signUpSuccess"), "success");
        if (!result.needsEmailConfirmation) {
          navigate("/dashboard");
        }
        return;
      }

      await signInUser(email, password);
      showToast(t("login.loginSuccess"), "success");
      navigate("/dashboard");
    } catch (err: any) {
      console.error(err);
      showToast(err.message || t("login.authFailure"), "error");
    } finally {
      setAuthLoading(false);
    }
  };

  const handleForgotPassword = async () => {
    if (!email.trim()) {
      showToast(t("login.resetMissingEmail"), "warning");
      return;
    }

    setAuthLoading(true);
    try {
      await requestPasswordReset(email);
      showToast(t("login.resetSent"), "success");
    } catch (err: any) {
      console.error(err);
      showToast(err.message || t("login.resetError"), "error");
    } finally {
      setAuthLoading(false);
    }
  };

  const handleSocialAuth = async (provider: "google" | "apple") => {
    setAuthLoading(true);
    try {
      await loginWithProvider(provider);
      showToast(t("login.socialSuccess", { provider: provider === "google" ? "Google" : "Apple" }), "success");
    } catch (err: any) {
      console.error(err);
      showToast(err.message || t("login.socialFailure"), "error");
    } finally {
      setAuthLoading(false);
    }
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] bg-darkbg text-white p-4 relative overflow-hidden">
      
      {/* Background ambient neon lighting */}
      <div className="absolute top-1/4 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 rounded-full filter blur-[120px] bg-volt/10 pointer-events-none" />
      <div className="absolute bottom-1/4 left-1/3 w-72 h-72 rounded-full filter blur-[100px] bg-hyperpink/10 pointer-events-none" />

      {/* Cyber title */}
      <div className="flex flex-col items-center gap-1 mb-8 text-center select-none z-10 animate-fade-in">
        <div className="flex items-center gap-2">
          <div className="bg-volt/10 border border-volt/30 p-2 rounded-2xl shadow-[0_0_15px_rgba(198,255,0,0.25)]">
            <Flame className="h-7 w-7 text-volt animate-pulse fill-current" />
          </div>
          <h1 className="text-4xl font-black tracking-tighter text-white uppercase italic">
            VELO<span className="text-volt drop-shadow-[0_0_8px_#C6FF00]">ZTY</span>
          </h1>
        </div>
        <span className="text-[9px] font-black tracking-[0.25em] text-mutedgray uppercase mt-1">
          {t("app.slogan")}
        </span>
      </div>

      <Card glow={isSignUp ? "pink" : "volt"} className="w-full max-w-sm z-10 p-7">
        {isSignUp && (
          <div className="text-center mb-6">
            <CardTitle className="text-xl">
              {t("login.signUpTitle")}
            </CardTitle>
            <p className="text-xs text-mutedgray mt-1">
              {t("login.signUpSubtitle")}
            </p>
          </div>
        )}

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          
          {/* Email input */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-bold text-mutedgray uppercase tracking-wider">{t("login.email")}</label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="piloto@velozty.com"
              required
              className="px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-sm font-semibold text-white focus:outline-none focus:border-volt focus:ring-1 focus:ring-volt tracking-wide placeholder-white/20 transition-all font-mono"
            />
          </div>

          {/* Display Name Input (Only on Sign Up) */}
          {isSignUp && (
            <div className="flex w-full flex-col gap-1.5 animate-slide-up">
              <label className="text-[10px] font-bold text-mutedgray uppercase tracking-wider">{t("login.nickname")}</label>
              <input
                type="text"
                value={displayName}
                onChange={(e) => setDisplayName(e.target.value)}
                placeholder="Ex: BoltVolt_99"
                required
                className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-sm font-semibold text-white focus:outline-none focus:border-hyperpink focus:ring-1 focus:ring-hyperpink tracking-wide placeholder-white/20 transition-all"
              />
            </div>
          )}

          {/* Password Input */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-bold text-mutedgray uppercase tracking-wider font-mono">{t("login.password")}</label>
            <input
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder="••••••••"
              required
              className="px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-sm font-semibold text-white focus:outline-none focus:border-volt focus:ring-1 focus:ring-volt tracking-wide placeholder-white/20 transition-all font-mono"
            />
          </div>

          {isSignUp && (
            <div className="flex w-full flex-col gap-1.5 animate-slide-up">
              <label className="text-[10px] font-bold text-mutedgray uppercase tracking-wider font-mono">{t("login.confirmPassword")}</label>
              <input
                type="password"
                value={confirmPassword}
                onChange={(e) => setConfirmPassword(e.target.value)}
                placeholder="••••••••"
                required
                className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-sm font-semibold text-white focus:outline-none focus:border-hyperpink focus:ring-1 focus:ring-hyperpink tracking-wide placeholder-white/20 transition-all font-mono"
              />
            </div>
          )}

          {!isSignUp && (
            <div className="-mt-1 text-right">
              <button
                type="button"
                onClick={handleForgotPassword}
                className="text-[10px] text-mutedgray hover:text-white transition-colors uppercase font-bold tracking-wider underline focus:outline-none"
              >
                {t("login.forgotPassword")}
              </button>
            </div>
          )}

          {isSignUp && (
            <label className="flex items-start gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-left animate-slide-up">
              <input
                type="checkbox"
                checked={acceptedTerms}
                onChange={(event) => setAcceptedTerms(event.target.checked)}
                required
                className="mt-0.5 h-4 w-4 rounded border-white/20 accent-volt"
              />
              <span className="text-[10px] font-semibold leading-relaxed text-mutedgray">
                {t("login.termsAccept")}
                <span className="block mt-1 text-[9px] text-mutedgray/80">
                  {t("login.termsPrivacySummary")}
                </span>
              </span>
            </label>
          )}

          {/* Action triggers */}
          <Button
            type="submit"
            variant={isSignUp ? "pink" : "volt"}
            isLoading={authLoading}
            disabled={isSignUp && !acceptedTerms}
            className="mt-2"
          >
            {isSignUp ? (
              <span className="flex items-center gap-1.5">
                <UserPlus className="h-4.5 w-4.5" />
                {t("login.signUpAction")}
              </span>
            ) : (
              <span className="flex items-center gap-1.5">
                <ShieldCheck className="h-4.5 w-4.5" />
                {t("login.signInAction")}
              </span>
            )}
          </Button>

          {/* Social Auth Section */}
          <div className="flex flex-col gap-3 mt-2">
            <div className="flex items-center justify-between gap-2">
              <div className="h-[1px] bg-white/10 flex-1" />
              <span className="text-[9px] font-black text-mutedgray uppercase tracking-widest">{t("login.orSignInWith")}</span>
              <div className="h-[1px] bg-white/10 flex-1" />
            </div>

            <div className="flex flex-col gap-3">
              <button
                type="button"
                onClick={() => handleSocialAuth("google")}
                disabled={authLoading}
                className="flex items-center justify-center gap-2 py-2.5 px-4 bg-white/5 hover:bg-white/10 border border-white/15 hover:border-volt/40 text-xs font-bold rounded-xl text-white transition-all active:scale-[0.98] cursor-pointer w-full"
              >
                <svg className="h-4 w-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                  <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" fill="#FBBC05"/>
                  <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" fill="#EA4335"/>
                </svg>
                <span>Google</span>
              </button>
            </div>
          </div>

          {/* Sign Up / Sign In switch toggler */}
          <div className="text-center mt-3">
            <button
              type="button"
              onClick={() => setIsSignUp(!isSignUp)}
              className="text-xs text-mutedgray hover:text-white transition-colors uppercase font-bold tracking-wider underline focus:outline-none"
            >
              {isSignUp ? t("login.haveAccount") : t("login.createAccount")}
            </button>
          </div>
        </form>
      </Card>

      {/* Cyber footer info */}
      <div className="mt-8 flex items-center gap-1.5 opacity-40 select-none">
        <Cpu className="h-3.5 w-3.5" />
        <span className="text-[8px] font-black tracking-widest text-mutedgray uppercase">
          {t("login.secureMode")}
        </span>
      </div>
    </div>
  );
};
export default Login;
